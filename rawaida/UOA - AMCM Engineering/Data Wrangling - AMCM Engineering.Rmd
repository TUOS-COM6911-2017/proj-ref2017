---
title: "Data Wrangling and Data Management for Aeronautical, Mechanical, Chemical and Manufacturing Engineering"
output: html_notebook
---

Units of Assessment Studied: Aeronautical, Mechanical, Chemical and Manufacturing Engineering - UOA 12
...
library(readr)
StaffSummary <- read_csv("~/03 COM6911 ITP/Data/Selected/12 - Aeronautical, Mechanical, Chemical and Manufacturing Engineering (1)/AMCM Engineering_StaffSummary.csv", 
    col_types = cols(TotalCategoryCHeadCount = col_skip(), 
...
library(readr)
SubmissionProfile <- read_csv("~/03 COM6911 ITP/Data/Selected/12 - Aeronautical, Mechanical, Chemical and Manufacturing Engineering (1)/AMCM Engineering_SubmissionProfile.csv", 
    col_types = cols(UnitOfAssessment = col_skip()))
...
library(readr)
Output <- read_csv("~/03 COM6911 ITP/Data/Selected/12 - Aeronautical, Mechanical, Chemical and Manufacturing Engineering (1)/AMCM Engineering_Output.csv", 
...
# Merge multiple submission profile so that there's only unique value for each university/ UKPRN
Output_Rank_X <- Output_Rank[!(is.na(Output_Rank$MultipleSubmission)),]
Output_Rank_Y <- Output_Rank[(is.na(Output_Rank$MultipleSubmission)),]

Output_Rank_X$FourStar[c(1, 3, 5)] <- Output_Rank_X$FourStar[Output_Rank_X$MultipleSubmission == 'A'] + Output_Rank_X$FourStar[Output_Rank_X$MultipleSubmission == 'B']

Output_Rank_X$ThreeStar[c(1, 3, 5)] <- Output_Rank_X$ThreeStar[Output_Rank_X$MultipleSubmission == 'A'] + Output_Rank_X$ThreeStar[Output_Rank_X$MultipleSubmission == 'B']

Output_Rank_X <- Output_Rank_X[c(1,3,5),]

Output_Rank_X$FourStar <- Output_Rank_X$FourStar / 2
Output_Rank_X$ThreeStar <- Output_Rank_X$ThreeStar / 2

Output_Rank <- rbind(Output_Rank_X, Output_Rank_Y)
...
e) Calculate average of FTE that has multiple submissions

```{r}
# Calculate average of each dataset if the university has multiple submissions

FTE_X <- FTE[!(is.na(FTE$MultipleSubmission)),]
FTE_Y <- FTE[(is.na(FTE$MultipleSubmission)),]

FTE_X$TotalCategoryAFte[c(1, 3, 5, 7, 9, 11)] <- FTE_X$TotalCategoryAFte[FTE_X$MultipleSubmission == 'A'] + FTE_X$TotalCategoryAFte[FTE_X$MultipleSubmission == 'B']

FTE_X <- FTE_X[c(1,3,5,7,9,11),]

FTE_X$TotalCategoryAFte <- FTE_X$TotalCategoryAFte / 2

FTE <- rbind(FTE_X, FTE_Y)
